...
public class TCPReactor implements Runnable {
    ...
    public TCPReactor(int port) throws IOException {
        ...
        serverSocket.bind(addr); // 在ServerSocketChannel綁定監聽端口
    ...
    private void dispatch(SelectionKey key) {
        if(key.attachment() instanceof TCPHandler){
            TCPHandler r = (TCPHandler) (key.attachment()); // 根據事件之key綁定的對象開新線程
            if (r != null)
                r.run();
        }else if(key.attachment() instanceof Acceptor){
            Acceptor r = (Acceptor) (key.attachment()); // 根據事件之key綁定的對象開新線程
            if (r != null)
                r.run();
        }
